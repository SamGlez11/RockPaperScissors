import pygame
from NetworkConfig import NetworkRemote
from player import Player
import sys

# Get host IP from command line argument
if len(sys.argv) != 2:
    print("Usage: python ClientRemote.py <host_ip>")
    print("Example: python ClientRemote.py 172.25.27.124")
    sys.exit(1)
...
pygame.display.set_caption(f"Remote Player - Connected to {host_ip}")
...
def redrawWindow(win, game, p):
    ...
    if not (game.connected()):
        ...
        text = font.render("Connecting to host...", True, (255, 0, 0))
        win.blit(text, (width/2 - text.get_width()/2, height/2 - text.get_height()/2))
        
        font_small = pygame.font.SysFont("Lexend", 30)
        ip_text = font_small.render(f"Host IP: {host_ip}", True, (0, 255, 0))
        ...
        text = font.render("Host Player", True, (0, 255, 255))
...
def main():
    ...
    try:
        n = NetworkRemote(host_ip)  # Connect to host's IP
        player = int(n.getP())
        print(f"Connected to host at {host_ip}")
        print("You are player", player)
    except Exception as e:
        print(f"Failed to connect to host at {host_ip}: {e}")
        return
